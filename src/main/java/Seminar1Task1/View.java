...
import Seminar1Task1.Units.Player;
...
public class View {
    ...
    private static String getChar(int x, int y){
        String out = "| ";
        for (Player player: main.team3) {
            if (player.getCoords()[0] == x && player.getCoords()[1] == y){
                if (player.getHp() == 0) {
                    out = "|" + (AnsiColors.ANSI_RED + player.toString().charAt(0) + AnsiColors.ANSI_RESET);
                    break;
                }
                if (main.team1.contains(player)) out = "|" + (AnsiColors.ANSI_GREEN + player.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                if (main.team2.contains(player)) out = "|" + (AnsiColors.ANSI_BLUE + player.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                break;
            }
        }
        return out;
